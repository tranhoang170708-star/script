try {
  ...
  // Chỉ kích hoạt khi game load các tệp dữ liệu chính (lúc đã vào sảnh/nhân vật)
  if (r.includes("assetindexer") || r.includes("CheckHackBehavior") || r.includes("GetBackpack")) {
    
    // Ghi nhật ký vào Shadowrocket Log để kiểm tra code chạy hay không
    console.log(">>> [PREDATOR XITS] DA VAO SAU: SCRIPT DANG KICH HOAT <<<");
    
    if (typeof b === 'string' && b.length > 0) {
      // Các lệnh can thiệp thông số
      ...
      // Xử lý header để game nhận dữ liệu mới
      if (h) {
        delete h['ETag'];
        delete h['etag'];
    ...
    // Không phải file mục tiêu thì để game chạy bình thường
    $done({});
  }
} catch (e) {
  // Bắt lỗi để không làm văng game
  $done({});
}
